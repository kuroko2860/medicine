...
async function createBatch(batch) {
  ...
  let result = await pool
    ...
    .query(
      "INSERT INTO medicine_batches (medicine_id, batch_number, expiration_date, quantity, date_received) VALUES (@medicine_id, @batch_number, @expiration_date, @quantity, @date_received)"
    );
  return result;
}
...
async function updateBatch(batch_number, batch) {
  ...
  let result = await pool
    ...
    .input("date_received", sql.Date, batch.date_received)
    .query(
      "UPDATE medicine_batches SET medicine_id = @medicine_id, expiration_date = @expiration_date, quantity = @quantity, date_received = @date_received WHERE batch_number = @batch_number"
    );
  return result;
}
